...
const PayrollsPage = () => {
  ...
  const filteredData = data.filter((employee) => {
    ...
    const monthMatch = filters.month === '' || employee.Mes.toLowerCase() === filters.month.toLowerCase();

    return nameMatch && addressMatch && monthMatch;
  });
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Sueldo bruto aproximado
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Sueldo neto aproximado
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {currentItems.map((employee, index) => (
              <tr key={index}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{employee.Sueldo}</td>
                <td className="px-6 py-4 whitespace-nowrap">{employee['*NETO*']}</td>
              </tr>
            ))}
          </tbody>
        </table>
